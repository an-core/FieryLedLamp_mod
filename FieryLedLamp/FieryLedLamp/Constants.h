...
// -----------------------------------------------------------------------------------------------------------------------------------------------------
// ДЛЯ ESP32-S3: ПЛЕЕР, ДИСПЛЕЙ (TM1637 / ST7789), ПУЛЬТ, КНОПКА, SD - ОТКЛЮЧЕНИЕ И ВКЛЮЧЕНИЕ ОСУЩЕСТВЛЯЕТСЯ В ВЭБ-ИНТЕРФЕЙСЕ
// ДЛЯ ESP32: ПЛЕЕР, ДИСПЛЕЙ (TM1637 / ST7789), ПУЛЬТ, КНОПКА, SD - ОТКЛЮЧЕНЫ ПО УМОЛЧАНИЮ (ТАКЖЕ НЕОБХОДИМО УКАЗАТЬ #define USE_OTA 0 и Partition Scheme: No OTA (2MB APP / 2MB SPIFFS))
// ** если вы захотите для ESP32 включить, например, SD карту для .out эффектов, то измените значение константы в файле ModuleConfig.h
...
// --------------------------------------------------------------------------------------------------
#define FASTLED_INTERRUPT_RETRY_COUNT (2) // Количество попыток повторной передачи кадра
...
#define DYNAMIC_DELAY_TICK  if (millis() - effTimer >= (256U - constrain(modes[currentMode].Speed, 1, 255)))
#define HIGH_DELAY_TICK     if (millis() - effTimer >= HIGH_DELAY)
#define LOW_DELAY_TICK      if (millis() - effTimer >= LOW_DELAY)
...
#define EEPROM_FIRST_RUN_MARK ((uint8_t)(MODE_AMOUNT - 255)) // метка первого запуска
#define EEPROM_WRITE_DELAY (30000UL)               // задержка перед записью в EEPROM (30 секунд)
#define SAVE_FILE_DELAY_TIMEOUT (5000UL)           // задержка перед сохранением в файл (15 секунд)
// ------------------
#if BACKUP_CFG_FILES
